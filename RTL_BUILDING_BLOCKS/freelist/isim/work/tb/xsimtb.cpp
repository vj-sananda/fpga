...
va_list vap)
{
    extern HSim__s6 * createworkMdpmem(const char*);
    HSim__s6 *blk = createworkMdpmem(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMfreelist(const char*);
    HSim__s6 *blk = createworkMfreelist(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("dpmem", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("freelist", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
