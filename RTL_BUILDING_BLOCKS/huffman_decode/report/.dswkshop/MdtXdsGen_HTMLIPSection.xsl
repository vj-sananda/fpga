...
<xsl:template name="Peri_PinoutTable">
	...
	<TABLE BGCOLOR="{$DS_COL_BLACK}" WIDTH="{ceiling($DS_WIDTH div 2)}" COLS="7" cellspacing="1" cellpadding="1" border="0">
		<TH COLSPAN="7" width="100%" align="middle" bgcolor="{$DS_COL_XPRP}"><SPAN style="color:{$DS_COL_WHITE}; font: bold 9px Verdana,Arial,Helvetica,sans-serif">PINOUT</SPAN></TH>
	 	<TR></TR>	
		...
		<TH COLSPAN="1" width="10%" align="middle" bgcolor="{$DS_COL_GREY}"><SPAN style="color:{$DS_COL_XPRP}; font: bold 10px Verdana,Arial,Helvetica,sans-serif">[MSB:LSB]</SPAN></TH>
		...
		<xsl:for-each select="MPORT">
			...
			<xsl:if test="@MSB and @LSB">
				<TD COLSPAN="1" width="10%" align="middle" bgcolor="{$DS_COL_WHITE}"><SPAN style="color:{$DS_COL_BLACK}; font: bold 10px Verdana,Arial,Helvetica,sans-serif"><xsl:value-of select="@MSB"/>:<xsl:value-of select="@LSB"/></SPAN></TD>
			</xsl:if>			
		...
		<xsl:if test="BUSINTERFACE">
			...
			<TH COLSPAN="1" width="15%" align="middle" bgcolor="{$DS_COL_GREY}"><SPAN style="color:{$DS_COL_XPRP}; font: bold 10px Verdana,Arial,Helvetica,sans-serif">TYPE</SPAN></TH>
		...
		<xsl:if test="INTERRUPTSRCS">
			...
			<TH COLSPAN="7" width="100%" align="middle" bgcolor="{$DS_COL_XPRP}">
				...
				<TR></TR>
				<TH COLSPAN="1" width="10%" align="middle" bgcolor="{$DS_COL_GREY}"><SPAN style="color:{$DS_COL_XPRP}; font: bold 10px Verdana,Arial,Helvetica,sans-serif">#</SPAN></TH>
				...
				<xsl:sort data-type="number" select="@INTRINDEX" order="ascending"/>
				
					<xsl:variable name="intrsrc_"><xsl:value-of select="@SRCSIG"/></xsl:variable>
					<xsl:variable name="intrnum_"><xsl:value-of select="@INTRINDEX"/></xsl:variable>
					
					<xsl:for-each select="../../../MODULE[MPORT[@DIR='O' and @SIGNAME = $intrsrc_]]">
						<TR></TR>
						<TH COLSPAN="1" width="10%" align="middle" bgcolor="{$DS_COL_WHITE}"><SPAN style="color:{$DS_COL_BLACK}; font: normal 12px Verdana,Arial,Helvetica,sans-serif"><xsl:value-of select="$intrnum_"/></SPAN></TH>
