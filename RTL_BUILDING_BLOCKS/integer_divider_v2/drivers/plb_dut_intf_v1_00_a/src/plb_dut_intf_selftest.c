//////////////////////////////////////////////////////////////////////////////
// Filename:          E:\work\FPGA_PROJECTS\BLOCKS\integer_divider_v2/drivers/plb_dut_intf_v1_00_a/src/plb_dut_intf_selftest.c
// Version:           1.00.a
// Description:       Contains a diagnostic self-test function for the plb_dut_intf driver
...
XStatus PLB_DUT_INTF_SelfTest(void * baseaddr_p)
{
  ...
  PLB_DUT_INTF_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = PLB_DUT_INTF_mReadSlaveReg0(baseaddr, 0);
  ...
  PLB_DUT_INTF_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = PLB_DUT_INTF_mReadSlaveReg1(baseaddr, 0);
  ...
  PLB_DUT_INTF_mResetWriteFIFO(baseaddr);
  xil_printf("   - reset read packet FIFO to initial state \n\r");
  PLB_DUT_INTF_mResetReadFIFO(baseaddr);
  xil_printf("   - push data to write packet FIFO\n\r");
  for ( Index = 0; Index < 4; Index++ )
  {
    xil_printf("     0x%08x", Index*1+1);
    PLB_DUT_INTF_mWriteToFIFO(baseaddr, 0, Index*1+1);
    xil_printf("\n\r");
  }
  xil_printf("   - write packet FIFO is %s\n\r", ( PLB_DUT_INTF_mWriteFIFOFull(baseaddr) ? "full" : "not full" ));
  xil_printf("   - pop data out from read packet FIFO\n\r");
  for ( Index = 0; Index < 4; Index++ )
  {
    Reg32Value = PLB_DUT_INTF_mReadFromFIFO(baseaddr, 0);
  ...
  xil_printf("   - read packet FIFO is %s\n\r", ( PLB_DUT_INTF_mReadFIFOEmpty(baseaddr) ? "empty" : "not empty" ));
  PLB_DUT_INTF_mResetWriteFIFO(baseaddr);
  PLB_DUT_INTF_mResetReadFIFO(baseaddr);
