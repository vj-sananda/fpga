//////////////////////////////////////////////////////////////////////////////
// Filename:          E:\work\FPGA_PROJECTS\BLOCKS\integer_divider/drivers/integer_divider_v1_00_a/src/integer_divider_selftest.c
// Version:           1.00.a
// Description:       Contains a diagnostic self-test function for the integer_divider driver
...
XStatus INTEGER_DIVIDER_SelfTest(void * baseaddr_p)
{
  ...
  INTEGER_DIVIDER_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = INTEGER_DIVIDER_mReadSlaveReg0(baseaddr, 0);
  ...
  INTEGER_DIVIDER_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = INTEGER_DIVIDER_mReadSlaveReg1(baseaddr, 0);
  ...
  INTEGER_DIVIDER_mResetWriteFIFO(baseaddr);
  xil_printf("   - reset read packet FIFO to initial state \n\r");
  INTEGER_DIVIDER_mResetReadFIFO(baseaddr);
  xil_printf("   - push data to write packet FIFO\n\r");
  for ( Index = 0; Index < 4; Index++ )
  {
    xil_printf("     0x%08x", Index*1+1);
    INTEGER_DIVIDER_mWriteToFIFO(baseaddr, 0, Index*1+1);
    xil_printf("\n\r");
  }
  xil_printf("   - write packet FIFO is %s\n\r", ( INTEGER_DIVIDER_mWriteFIFOFull(baseaddr) ? "full" : "not full" ));
  xil_printf("   - pop data out from read packet FIFO\n\r");
  for ( Index = 0; Index < 4; Index++ )
  {
    Reg32Value = INTEGER_DIVIDER_mReadFromFIFO(baseaddr, 0);
  ...
  xil_printf("   - read packet FIFO is %s\n\r", ( INTEGER_DIVIDER_mReadFIFOEmpty(baseaddr) ? "empty" : "not empty" ));
  INTEGER_DIVIDER_mResetWriteFIFO(baseaddr);
  INTEGER_DIVIDER_mResetReadFIFO(baseaddr);
