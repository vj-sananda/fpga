//////////////////////////////////////////////////////////////////////////////
// Filename:          E:\work\FPGA_PROJECTS\ML509\ml509_dpi/drivers/kraaken_dpi_v1_00_a/src/kraaken_dpi_selftest.c
// Version:           1.00.a
// Description:       Contains a diagnostic self-test function for the kraaken_dpi driver
// Date:              Tue Mar 30 00:37:04 2010 (by Create and Import Peripheral Wizard)
//////////////////////////////////////////////////////////////////////////////
...
XStatus KRAAKEN_DPI_SelfTest(void * baseaddr_p)
{
  ...
  KRAAKEN_DPI_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = KRAAKEN_DPI_mReadSlaveReg0(baseaddr, 0);
  ...
  KRAAKEN_DPI_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = KRAAKEN_DPI_mReadSlaveReg1(baseaddr, 0);
  ...
  KRAAKEN_DPI_mWriteSlaveReg2(baseaddr, 0, 3);
  Reg32Value = KRAAKEN_DPI_mReadSlaveReg2(baseaddr, 0);
  ...
  KRAAKEN_DPI_mWriteSlaveReg3(baseaddr, 0, 4);
  Reg32Value = KRAAKEN_DPI_mReadSlaveReg3(baseaddr, 0);
  ...
  Reg32Value = KRAAKEN_DPI_mReadReg(baseaddr, KRAAKEN_DPI_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  KRAAKEN_DPI_mWriteReg(baseaddr, KRAAKEN_DPI_INTR_IPISR_OFFSET, Reg32Value);
  Reg32Value = KRAAKEN_DPI_mReadReg(baseaddr, KRAAKEN_DPI_INTR_DISR_OFFSET);
  xil_printf("   - Device (peripheral) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear Device (peripheral) interrupt status register\n\r");
  KRAAKEN_DPI_mWriteReg(baseaddr, KRAAKEN_DPI_INTR_DISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  KRAAKEN_DPI_EnableInterrupt(baseaddr_p);
