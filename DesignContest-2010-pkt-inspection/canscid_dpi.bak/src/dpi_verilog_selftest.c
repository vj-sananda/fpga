//////////////////////////////////////////////////////////////////////////////
// Filename:          E:\work\FPGA_PROJECTS\ML509\ml509_dma/drivers/dpi_verilog_v1_00_a/src/dpi_verilog_selftest.c
// Version:           1.00.a
// Description:       Contains a diagnostic self-test function for the dpi_verilog driver
...
XStatus DPI_VERILOG_SelfTest(void * baseaddr_p)
{
  ...
  DPI_VERILOG_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = DPI_VERILOG_mReadSlaveReg0(baseaddr, 0);
  ...
  DPI_VERILOG_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = DPI_VERILOG_mReadSlaveReg1(baseaddr, 0);
  ...
  DPI_VERILOG_mWriteSlaveReg2(baseaddr, 0, 3);
  Reg32Value = DPI_VERILOG_mReadSlaveReg2(baseaddr, 0);
  ...
  DPI_VERILOG_mWriteSlaveReg3(baseaddr, 0, 4);
  Reg32Value = DPI_VERILOG_mReadSlaveReg3(baseaddr, 0);
  ...
  Reg32Value = DPI_VERILOG_mReadReg(baseaddr, DPI_VERILOG_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  DPI_VERILOG_mWriteReg(baseaddr, DPI_VERILOG_INTR_IPISR_OFFSET, Reg32Value);
  Reg32Value = DPI_VERILOG_mReadReg(baseaddr, DPI_VERILOG_INTR_DISR_OFFSET);
  xil_printf("   - Device (peripheral) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear Device (peripheral) interrupt status register\n\r");
  DPI_VERILOG_mWriteReg(baseaddr, DPI_VERILOG_INTR_DISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  DPI_VERILOG_EnableInterrupt(baseaddr_p);
