...
va_list vap)
{
    extern HSim__s6 * createworkMapp__unpack(const char*);
    HSim__s6 *blk = createworkMapp__unpack(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMapp__pack(const char*);
    HSim__s6 *blk = createworkMapp__pack(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("app_unpack", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("app_pack", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
