...
int main() {
  ...
  for (i=0;i<4;i++) {
    ...
    initializedb_rand(globalkey,pwr);

    xil_printf("Sorting starts\n\r");

    start = ts_get_ll();

    //Uncomment 1 sort method
    //sortrecord();
    //sortrecord_blockmerge();
    sortrecord_hw_plb(pwr);

    stop = ts_get_ll();
  
    xil_printf("Elapsed Sorting Time %d us\n\r", ts_elapse_us(start, stop));
    xil_printf("Verifying results .. %d(correct = 1): ",pwr);
    xil_printf("%x\n\r", verifydb_tc(pwr));
