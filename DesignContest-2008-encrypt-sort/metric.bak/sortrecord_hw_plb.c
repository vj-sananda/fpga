...
void sortrecord_hw_plb(unsigned  pwr) {
  ...
  for ( i=0;i<num_fifo_fills;i++) {
    ...
    for( j=0; j<records_per_fill; j++ ) {
      ...
      while ( ACCEL_SORT_PLB_mReadFIFOEmpty(XPAR_ACCEL_SORT_PLB_0_BASEADDR) ) {	  	xil_printf("**** read fifo empty ********\n\r");
      }
  ...
  //blocksize is 2.
  rval = merge_all_sorted_blocks(BLOCKSIZE,pwr);
  ...
  for ( i=0;i<num_fifo_fills;i++) {
    for( j=0; j<records_per_fill; j++ ) {
      ...
      if ( rval ) {
	r.f1 = Mergeddb[ i*records_per_fill + j ].f1;
	r.f2 = Mergeddb[ i*records_per_fill + j ].f2;
	r.f3 = Mergeddb[ i*records_per_fill + j ].f3;
	r.f4 = Mergeddb[ i*records_per_fill + j ].f4;
      }
      else {
